...
class Graph:
    ...
    class Node:
        ...
        def _addConnection(self, to):
            # i.e. not already used in gcode generation
    ...
    def saveAsGcodeFile(self, f):
        ...
        def pathGcode(i, insidePath):
            ...
            x_values.append(self[i].y)
            y_values.append(self[i].x)
            commands.append(1 if insidePath else 0)
...
class EdgesToGcode:
    ...
    def getCircularArray(self, center, r, smallerArray=None):
        ...
        for i in range(circumferenceSize):
            ...
            if x not in range(self.xSize) or y not in range(self.ySize):
                # consider pixels outside of the image as not-edges
                circularArray[i] = False
            else:
                ...
                if smallerArray[a] == False and (b not in range(smallerSize) or smallerArray[b] == False):
                    # do not take into consideration not connected regions (roughly)
                    circularArray[i] = False
                else:
    ...
    def toCircularRanges(self, circularArray):
        ...
        ranges.append(CircularRange(lastValueIndex,
                      circumferenceSize, lastValue))
    ...
    def getNextPoints(self, point):
        ...
        for circularRange in circularRanges:
            if circularRange.value == True:
                ...
                x = point[0] + \
                    constants.circumferences[bestRadius][circumferenceIndex][0]
                y = point[1] + \
                    constants.circumferences[bestRadius][circumferenceIndex][1]
    ...
    def propagate(self, point, currentNodeIndex):
        ...
        def setSeenDFS(x, y):
            if (x in range(self.xSize) and y in range(self.ySize)
                ...
                # index of just added node
                self.ownerNode[x, y] = currentNodeIndex
                setSeenDFS(x+1, y)
                setSeenDFS(x-1, y)
                setSeenDFS(x, y+1)
                setSeenDFS(x, y-1)
...
def convertToBinaryEdges(edges, threshold):
    result = np.maximum.reduce(
        [edges[:, :, 0], edges[:, :, 1], edges[:, :, 2]]) >= threshold
